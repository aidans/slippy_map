...
public class SlippyMapTest extends PApplet{
	...
	public void draw(){
		...
		fill(0);
		text("lon="+slippyMap.getLonLatFromScreenCoord(mouseX,mouseY).x+"; lat="+slippyMap.getLonLatFromScreenCoord(mouseX,mouseY).y,0,height);
	}
	
	public void keyPressed(){
		if (key=='t'){
			//switch through the various map types
			if (slippyMap.getMapType()==SlippyMapType.BING_AERIAL)
			...
			else if (slippyMap.getMapType()==SlippyMapType.BING_ROAD)
				slippyMap.setMapType(SlippyMapType.STAMEN_WATERCOLOUR);
			else if (slippyMap.getMapType()==SlippyMapType.STAMEN_WATERCOLOUR)
				slippyMap.setMapType(SlippyMapType.STAMEN_TERRAIN);
			else if (slippyMap.getMapType()==SlippyMapType.STAMEN_TERRAIN)
